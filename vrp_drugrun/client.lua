                                    ...
                                    if vilaflevere == true then
                                        if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 1268.6319580078,-1710.4681396484,54.771492004395, true) <= 5 then
                                            ...
                                            if IsControlJustReleased(0, 38) then 
                                                vilaflevere = false
                                                TaskStartScenarioInPlace(PlayerPedId(), 'PROP_HUMAN_BUM_BIN', 0, true)
                                                exports['mythic_progbar']:Progress({name = "firstaid_action", duration = 5000, label = "Aflevere kasse...", useWhileDead = false, canCancel = true, controlDisables = {disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = true}})
                                                Citizen.Wait(5000)
                                                ClearPedTasksImmediately(GetPlayerPed(-1))
                                                DeleteObject(madpak)
...
RegisterNUICallback("farve3", function (data, callback)
    if valgt == true then
        ...
        SetNewWaypoint(1477.3278808594,-1917.5895996094)
                        ...
                        if IsControlJustReleased(0, 38) then 
                            ...
                            if random == 1 then
                                ...
                                if random == 2 then
                                    ...
                                    tekst = false
                                    local aflevermdma = true
                                    Citizen.Wait(2500)
                                    Citizen.CreateThread(function()
                                        while true do
                                            Citizen.Wait(1)
                                            if aflevermdma == true then
                                                if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 1477.3278808594,-1917.5895996094,71.450965881348, true) <= 3 then
            ...
            if mdma == true then
                if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 1272.3948974609,-1711.5767822266,54.771438598633, true) <= 2 then
                    ...
                    if IsControlJustReleased(0, 38) then 
                        mdma = false
                        TaskStartScenarioInPlace(PlayerPedId(), 'PROP_HUMAN_BUM_BIN', 0, true)
                        exports['mythic_progbar']:Progress({name = "firstaid_action", duration = 5000, label = "Aflevere piller...", useWhileDead = false, canCancel = true, controlDisables = {disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = true}})
                        Citizen.Wait(5000)
                        ClearPedTasksImmediately(GetPlayerPed(-1))
